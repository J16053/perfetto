...
bool LayerContains(LayerDecoder& layer,
                   LayerDecoder& other,
                   geometry::Rect* crop) {
...
bool LayerOverlaps(LayerDecoder& layer,
                   LayerDecoder& other,
                   geometry::Rect* crop) {
  auto layer_bounds = layer::GetCroppedScreenBounds(layer, crop);
  auto other_bounds = layer::GetCroppedScreenBounds(other, crop);
...
VisibilityComputation& VisibilityComputation::SetStringPool(StringPool* pool) {
  pool_ = pool;  // required to embed parent id in "hidden by parent" reason

  flag_is_hidden_id_ = pool_->InternString(base::StringView("flag is hidden"));
  buffer_is_empty_id_ =
      pool_->InternString(base::StringView("buffer is empty"));
  alpha_is_zero_id_ = pool_->InternString(base::StringView("alpha is 0"));
  bounds_is_zero_id_ = pool_->InternString(base::StringView("bounds is 0x0"));
  crop_is_zero_id_ = pool_->InternString(base::StringView("crop is 0x0"));
  transform_is_invalid_id_ =
      pool_->InternString(base::StringView("transform is invalid"));
  no_effects_id_ = pool_->InternString(
      base::StringView("does not have color fill, shadow or blur"));
  empty_visible_region_id_ = pool_->InternString(base::StringView(
      "visible region calculated by Composition Engine is empty"));
  null_visible_region_id_ =
      pool_->InternString(base::StringView("null visible region"));
  occluded_id_ = pool_->InternString(base::StringView("occluded"));
...
std::unordered_map<int, VisibilityProperties> VisibilityComputation::Compute() {
  ...
  for (auto it = layers_top_to_bottom_->begin();
    ...
    auto crop = GetDisplayCrop(layer, snapshot_decoder_);
